...
对照拓扑图、点「下一步」逐步执行**真实**后端动作的演示页(共 15 步):组件注册 →
两侧获取 X.509-SVID → mTLS 互访 → hacker 攻击被拒 → 获取 JWT-SVID → 身份公网可验证(JWKS)
→ 零密钥云联邦(合法/冒充)→ 撤销注册后立即失效(既拿不到新 SVID,也访问不了 Entra)→
恢复 → 标注未来可平滑演进到 CyberArk Idira 的组件。X.509-SVID 用于 mTLS 数据面,
JWT-SVID 用于云联邦。
...
- 多语言:右上角可切换 **English / 简体中文 / 한국어**,默认 English(记忆在浏览器 localStorage)。
  界面文案、步骤标题/说明、以及第 6/8/12/14 步(`fetch-jwt-svid.sh`、`access-graph.sh` 走
  `UILANG`)与第 15 步(Idira 归属对照)的说明性输出都会随语言切换;真实工具输出
  (docker、curl、SPIRE 等)保持原样。
...
> 第 10 步会删除 `/agent` 注册条目、第 13 步会恢复。若演示中途退出,直接重跑第 13 步或
> `docker compose exec -T spire-server /opt/spire/scripts/register-entries.sh` 即可恢复。
> 第 15 步仅为架构说明(打印组件归属对照),不改动任何运行状态。
