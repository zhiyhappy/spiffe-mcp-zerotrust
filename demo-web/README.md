...
对照拓扑图、点「下一步」逐步执行**真实**后端动作的演示页(共 13 步):组件注册 →
两侧获取 SVID → mTLS 互访 → hacker 攻击被拒 → 零密钥云联邦(合法/冒充)→
...
> 第 9 步会删除 `/agent` 注册条目、第 11 步会恢复。若演示中途退出,直接重跑第 11 步或
> `docker compose exec -T spire-server /opt/spire/scripts/register-entries.sh` 即可恢复。
> 第 13 步仅为架构说明(打印组件归属对照),不改动任何运行状态。
