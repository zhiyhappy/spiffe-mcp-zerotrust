...
IDIRA_ZH = (
    ...
    "        └ 但部署方式升级:改由 Sidecar Injector 自动注入(见下方 🔵)\n"
    "    • Open WebUI\n"
    "        └ 业务应用本身\n"
    "    • Microsoft Entra ID\n"
    "        └ 云 IdP;作为被联邦的对端\n"
    ...
    f"{_SEP}\n"
    "\n"
    "🔵  Idira 原生增强 —— 自动化注入 + 面向资源的经纪(超出本 demo 现状)\n"
    "\n"
    "    • Sidecar Injector(基于 K8s Mutating Webhook)\n"
    "        └ 管理员只需在 Pod 上打一个 Annotation,系统即自动把\n"
    "          Authenticator / 凭据代理容器作为 sidecar 注入 Pod\n"
    "        └ Agent 侧的 envoy-client 不再手工部署 —— 由 Injector 自动挂载\n"
    "    • Identity Broker(面向 MCP Server 等资源)\n"
    "        └ MCP 等资源今后无需单独部署 sidecar\n"
    "        └ 作为 Agent 与 MCP 之间的 Broker:校验 agent 身份、\n"
    "          执行访问策略、转发 MCP 调用\n"
    "\n"
    f"{_RULE}\n"
    "  演进动作(最小改造)\n"
    ...
    "    4. Agent 侧 envoy-client 改由 Sidecar Injector 按 Annotation 自动注入,免手工部署\n"
    "    5. MCP 侧撤掉 envoy-server sidecar,统一走 Identity Broker 完成鉴权与转发\n"
    "\n"
...
IDIRA_EN = (
    ...
    "        └ But the deployment upgrades: injected automatically by the Sidecar Injector (see 🔵 below)\n"
    "    • Open WebUI\n"
    ...
    f"{_SEP}\n"
    "\n"
    "🔵  Idira-native enhancements -- automatic injection + resource-facing broker (beyond this demo's current state)\n"
    "\n"
    "    • Sidecar Injector (K8s Mutating Webhook)\n"
    "        └ Admins just add an Annotation to the Pod; the system auto-injects the\n"
    "          Authenticator / credential-proxy container as a sidecar into the Pod\n"
    "        └ The Agent-side envoy-client is no longer deployed by hand -- the Injector mounts it\n"
    "    • Identity Broker (for resources like the MCP Server)\n"
    "        └ MCP and similar resources no longer need their own sidecar\n"
    "        └ Acts as the broker between Agent and MCP: validates agent identity,\n"
    "          enforces access policy, and forwards the MCP call\n"
    "\n"
    f"{_RULE}\n"
    ...
    "    4. Have the Agent-side envoy-client auto-injected by the Sidecar Injector via Annotation -- no manual deploy\n"
    "    5. Drop the MCP-side envoy-server sidecar; route auth and forwarding through the Identity Broker\n"
    "\n"
...
IDIRA_KO = (
    ...
    "        └ 단 배포 방식은 업그레이드: Sidecar Injector가 자동 주입(아래 🔵 참조)\n"
    "    • Open WebUI\n"
    "        └ 비즈니스 앱 자체\n"
    "    • Microsoft Entra ID\n"
    "        └ 클라우드 IdP; 페더레이션 상대방\n"
    ...
    f"{_SEP}\n"
    "\n"
    "🔵  Idira 네이티브 강화 -- 자동 주입 + 리소스 대상 브로커(본 demo 현황을 넘어서)\n"
    "\n"
    "    • Sidecar Injector(K8s Mutating Webhook 기반)\n"
    "        └ 관리자가 Pod에 Annotation만 추가하면 시스템이 Authenticator /\n"
    "          자격증명 프록시 컨테이너를 sidecar로 Pod에 자동 주입\n"
    "        └ Agent 측 envoy-client는 더 이상 수동 배포 안 함 -- Injector가 자동 장착\n"
    "    • Identity Broker(MCP Server 등 리소스 대상)\n"
    "        └ MCP 등 리소스는 앞으로 별도 sidecar 배포 불필요\n"
    "        └ Agent와 MCP 사이의 Broker 역할: agent 신원 검증,\n"
    "          접근 정책 집행, MCP 호출 전달\n"
    "\n"
    f"{_RULE}\n"
    "  진화 단계(최소 변경)\n"
    ...
    "    4. Agent 측 envoy-client를 Sidecar Injector가 Annotation 기반으로 자동 주입 -- 수동 배포 불필요\n"
    "    5. MCP 측 envoy-server sidecar 제거; 인증과 전달을 Identity Broker로 일원화\n"
    "\n"
